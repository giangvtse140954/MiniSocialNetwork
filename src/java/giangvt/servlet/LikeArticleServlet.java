...
public class LikeArticleServlet extends HttpServlet {
    ...
    private final Logger LOGGER = Logger.getLogger(LikeArticleServlet.class);
        ...
        try {
            ...
            if (session != null) {
                ...
                if (postId == null) {
                    ...
                    // count = 0 khi dang trang thai dislike
                    if (count == 0) {
                        boolean isUpdated = dao.updateEmotion(email, Integer.parseInt(postId), true);
                        if (isUpdated) {
                            ...
                            notiDao.updateNoti(1, noId);
                            String urlRewriting = "ShowDetailServlet?txtPostId=" + postId;
                            url = urlRewriting;
                        }
                    } else if (count == 1) { // count = 1 khi trang thai like
                    ...
                    } else if (count == -1) { // count = -1 khi ko gi ca
                        long millis = System.currentTimeMillis();
                        Date date = new Date(millis);

                        TblNotifyDAO notiDao = new TblNotifyDAO();
                        int notifyId = notiDao.insertNoti(Integer.parseInt(postId), email, date + "", 1);
            ...
            LOGGER.info("LikeArticleServlet _ SQL: " + ex.getMessage());
        } catch (NamingException ex) {
            LOGGER.info("LikeArticleServlet _ Naming: " + ex.getMessage());
        } finally {
